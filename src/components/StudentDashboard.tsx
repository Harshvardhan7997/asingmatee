...
import { MOCK_ASSIGNMENTS, SUBJECTS } from '@/lib/mock-data';
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {SUBJECTS.map(sub => (
            <div key={sub.id} className="bg-card border border-border rounded-lg p-4 hover:border-primary/30 transition-colors cursor-pointer cyber-border">
              <div className={`text-${sub.color} text-lg mb-1`}>
            ...
            <div key={a.id} className="bg-card border border-border rounded-lg p-4 flex items-center justify-between cyber-border">
              <div>
                ...
                <div className="flex items-center gap-3 text-xs text-muted-foreground mt-1">
                  ...
                  <span className={`capitalize font-medium ${a.difficulty === 'easy' ? 'text-neon-green' : a.difficulty === 'medium' ? 'text-neon-yellow' : a.difficulty === 'hard' ? 'text-neon-orange' : 'text-neon-red'}`}>
                    {a.difficulty}
                  </span>
                  <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {format(new Date(a.dueDate), 'MMM dd, HH:mm')}</span>
                </div>
              </div>
